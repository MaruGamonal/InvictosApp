...
> **Revisión 17.** Entran los torneos con **varias categorías competitivas** —la A, la B, la C—. Es el primer pedido de esta serie que **sí necesita una entidad nueva**, pero no donde parecía: la categoría competitiva **no cabe adentro del torneo** porque está *por encima* de la Fase, no por debajo (D-103). Se resuelve con lo que D-19b ya había dejado previsto: **N torneos agrupados por un Certamen**, con la división como **etiqueta libre** (D-104), configuración **copiada y no compartida** (D-105) y divisiones **independientes entre sí** (D-106). Se completa con el agrupamiento en el descubrimiento (D-107), el rechazo con división sugerida (D-108) y el equipo en dos divisiones (D-109). Queda **una decisión abierta nueva**: el score no distingue nivel competitivo (P-52).
>
...
> **[Pendiente de definición] Quedan dos, y ninguna bloquea el MVP:**
>
> 1. **Desde la revisión 11 (P-51):** cómo se trata el **Gran Buenos Aires** en un modelo de una sola ciudad por persona (sección 4.11). No bloquea el desarrollo del MVP —el modelo de ciudad funciona igual—, pero **sí bloquea el diseño del selector de ciudad**, así que conviene resolverla antes de esa pantalla.
> 2. **Desde la revisión 17 (P-52):** el score y el ranking **no distinguen nivel competitivo**, así que el campeón de la categoría C y el de la A suman en el mismo ranking (sección 4.17). No es un problema nuevo —S-02 lo registró en la revisión 1—, pero las divisiones lo vuelven visible. **Bloquea el cálculo del primer score**, no el MVP, porque el score es de etapa futura (D-94).

**Fuera de esas dos, no quedan decisiones de negocio abiertas.** Las 39 pendientes que abrió la revisión 1 están todas cerradas: 27 en la revisión 2, 2 en la revisión 3 (monetización y permisos) y las 16 restantes en la revisión 4, al adoptarse todas las recomendaciones (D-51 a D-66, sección 4.4). En la revisión 5 se confirmaron además los 4 supuestos (D-67, D-68, sección 4.5).
...
|---|---|---|---|
| **Decisiones abiertas** | **2** | 4.11, 4.17 | **P-51**, el tratamiento del AMBA (bloquea el diseño del selector de ciudad) y **P-52**, el nivel competitivo en el score (bloquea el cálculo del primer score). **Ninguna bloquea el MVP** |
...
**[Definido] Las decisiones abiertas llevan `P-nn` y las cerradas `D-nn`**, y se numeran en el orden en que aparecen. Es la convención de la sección 8: una `P` que se resuelve pasa a la sección 4 **conservando su número**. Las dos abiertas de hoy son **P-51** y **P-52** — la del AMBA no tenía número hasta ahora, y se lo lleva para que se la pueda citar como a cualquier otra.
...
| **D-19b** *(era P-19)* | ¿Existen competencias recurrentes con ediciones? | **No en esta versión**: cada torneo es independiente. El modelo deja **previsto un vínculo opcional** hacia una competencia agrupadora | Permite agregarlo después sin migrar todos los dominios derivados. *(El vínculo previsto se construyó en la revisión 17, D-103, para agrupar **divisiones**. Las **ediciones** —el eje del que habla esta decisión— siguen sin modelarse: D-19b **no está superada**)* |
...
### 4.17 Decisiones de la revisión 17 — un torneo con varias categorías competitivas

**De dónde sale:** de que un mismo torneo puede abrirse con **varias categorías** —la A, la B, la C—. A diferencia de las cuatro revisiones anteriores, acá **el set no lo tenía cubierto**, y conviene decir con precisión por qué.

**La primera pregunta es dónde vive la categoría competitiva, y la respuesta descarta los dos candidatos obvios.** El modelo ya tiene dos niveles adentro del torneo —**Fase** y **Grupo** (`03`, 3.8)—, y ninguno sirve:

| Candidato | Por qué no |
|---|---|
| **Grupo** ("Zona A", "Zona B") | La zona es una división **dentro de una fase** y existe para repartir un fixture. La categoría A no es una zona de la B: **no se cruzan nunca y no comparten tabla** |
| **Fase** | La fase es **secuencial en el tiempo** (`Fase.orden`): grupos → cuartos → final. Las categorías corren **en paralelo**, y cada una tiene su propia secuencia completa de fases. La categoría está **por encima** de la fase, no por debajo |

**Y ahí está el punto:** el único nivel que hay por encima de la Fase es el **Torneo**. Una categoría competitiva tiene fixture propio, tabla propia, campeón propio, cupo propio y estado propio — que es, palabra por palabra, la definición de Torneo en este modelo. Entonces no hay que inventar un nivel: **hay que agrupar torneos.** Es exactamente el vínculo que D-19b dejó previsto y no construido.

| # | Pregunta | Decisión | Fundamento |
|---|---|---|---|
| **D-103** | ¿Un torneo con categorías A/B/C es un torneo o varios? | **Son varios.** Cada categoría es un **Torneo** completo, y las categorías de un mismo evento se agrupan con una entidad nueva: **`CERTAMEN`** *(así llamada, y no "Competencia", para no chocar con el dominio D7 — ver abajo)*, vinculado desde `TORNEO.certamen_id` — **opcional y vacío en el caso normal** (`03`, 3.23) | Es el corte que deja **todo lo derivado intacto**: fixture, tabla, posiciones, cupo, inscripción, score y campeón ya funcionan por torneo y no se tocan. La alternativa —meter un nivel entre Torneo y Fase— obliga a mover `Fase.torneo_id`, `Inscripcion.torneo_id`, `Posicion`, `Partido` y todo el ranking: es la cirugía más cara posible, en el núcleo competitivo, para modelar algo que ya es un torneo. Y **retoma D-19b** en vez de contradecirlo |
| **D-104** | ¿La división es una enumeración (A, B, C)? | **No: texto libre.** `TORNEO.division` es una **etiqueta**, no un valor de catálogo — "A", "Primera", "Oro", "Ascenso" | Es el criterio que este set ya aplica en `04`, 5.3: *lista cerrada solo donde el sistema compara*. Acá **no hay nada que comparar**: la "A" de un organizador no significa lo mismo que la "A" de otro, no hay ascensos ni descensos entre ellas (D-19b), y **ningún filtro ni ranking corta por división** (P-52). Una enumeración obligaría a llamar "A" a lo que el organizador llama "Oro", sin comprarle nada al sistema |
| **D-105** | ¿El Certamen guarda la configuración compartida? | **No. Guarda solo identidad** (nombre y organización). Lo compartido —ciudad, dirección, fechas, modalidad, categoría, reglamento y **colaboradores**— **se copia** al crear cada división, y desde ahí cada una es dueña de lo suyo | Dos fuentes de verdad para la misma ciudad o la misma fecha es una incoherencia esperando ocurrir, y **las divisiones legítimamente difieren**: la C juega sábados y la A domingos, el reglamento de la A pide 18 jugadores y el de la C 14. Copiar da toda la comodidad (el organizador no retipea nada) sin ninguna de las ataduras. Es además lo que hace que **la división no sea un caso especial** para ningún servicio: lee un torneo normal |
| **D-106** | ¿Las divisiones avanzan juntas? | **No. Cada división es independiente en estado, fixture, tabla, campeón y cupo.** La A puede estar `finished` con la C todavía `in_progress`. **El Certamen no tiene estado ni campeón** | Obligarlas a moverse juntas inventaría una máquina de estados nueva —la del grupo— que tendría que resolver qué pasa cuando una división se cancela y las otras siguen. No hay ningún caso real que lo pida: en la cancha las categorías se manejan por separado, y el organizador cierra la C cuando termina la C |
| **D-107** | ¿Cómo se ven en el descubrimiento? | **Agrupadas, sin cambiar la consulta.** El listado sigue devolviendo **torneos**; cuando varios de el mismo certamen caen juntos, la interfaz los muestra como **un bloque con el nombre del certamen y una división por fila** (`08`) | Tres tarjetas casi idénticas es exactamente el ruido que el descubrimiento existe para evitar (`12`). Agrupar en el render y no en la consulta lo resuelve **sin tocar los filtros ni la paginación**. **Limitación aceptada y explícita:** si el corte de página cae en el medio, el bloque se parte. Es un defecto cosmético de un caso poco frecuente, y el precio de evitarlo —un listado que devuelve competencias y hace coincidir "si alguna división coincide"— es rehacer la consulta más caliente del producto |
| **D-108** | Si un equipo se anota en la división equivocada, ¿el organizador lo reasigna? | **No: lo rechaza indicando la división que le corresponde**, con un motivo nuevo (`04`, 4.15), y el equipo se inscribe ahí | Dos razones, y la de producto pesa más que la técnica. **La de producto:** la división es un **juicio sobre el nivel del equipo**, y moverlo a una categoría más baja sin preguntarle es una decisión social que la app no debería tomar en silencio — el rechazo con sugerencia deja que el equipo la acepte. **La técnica:** la identidad de la Inscripción es `torneo + equipo` (`03`, 3.9), así que "reasignar" es borrar y recrear una fila con otra clave, arrastrando fechas y autoría a mano. Rechazar **no cuesta una línea de modelo** |
| **D-109** | ¿Un equipo puede anotarse en dos divisiones del mismo evento? | **Sí, pero avisa.** Son dos torneos distintos, así que estructuralmente ya se puede; el sistema lo **advierte** a quien inscribe y al organizador, y el organizador decide | Es literalmente la misma pregunta que D-82 (categoría cruzada) y D-16b (nombre duplicado), y merece la misma respuesta: **el reglamento del torneo manda y el organizador ya aprueba cada inscripción** (D-93). Bloquear convierte el caso legítimo —el club que manda su primera a la A y su reserva a la B, o el equipo que juega la libre y la de veteranos— en un pedido de soporte |

**[Pendiente de definición — P-52] El score y el ranking no distinguen nivel competitivo, y esta revisión lo deja a la vista.** El recorte del ranking es **ciudad + modalidad + categoría** (D-92), y la división **no entra**: no puede, porque es texto libre y local a un organizador (D-104). La consecuencia es que **el campeón de la C y el campeón de la A suman en el mismo ranking**. Conviene ser claro en dos cosas:

- **No es un problema que introduzcan las divisiones.** Ya existía entre dos torneos cualquiera de distinto nivel — el modelo **nunca** tuvo un concepto de nivel competitivo, y S-02 lo registró desde la revisión 1 ("¿cómo se comparan torneos de distinto nivel?" → *no se comparan*). Las categorías A/B/C solo lo vuelven visible **dentro de un mismo evento**, que es donde más incomoda.
- **No bloquea nada del MVP**, porque el score y los rankings son de etapa futura (D-94). Lo que sí hace es **ponerle fecha a la decisión**: hay que resolverla antes de calcular el primer score, no después.

**Caminos posibles, sin elegir ninguno:** inferir el nivel del torneo a partir del score de sus participantes (lo que S-02 ya dejó anotado como el camino cuando haya volumen); ponderar por nivel inferido en vez de recortar; o no hacer nada y aceptar que el score mide *rendimiento*, no *nivel*, y decirlo en el desglose (UC-40). **Lo que no sirve** es dejar que el organizador declare el nivel de su torneo: S-02 ya explicó por qué —todos declaran "alto"—.

**[Definido — D-103] El caso normal no se entera de nada de esto.** Un torneo sin categorías tiene `certamen_id` vacío y `division` vacía, y se comporta exactamente como antes. Es la misma disciplina con la que se agregaron el reglamento (opcional, D-29) y la lista de espera: **lo nuevo no puede encarecer el caso frecuente**. La interfaz de creación no menciona divisiones hasta que el organizador elige agregar una.

**[Definido — D-103] La entidad se llama `CERTAMEN` y no "Competencia", que era el nombre obvio.** El motivo es una colisión concreta: **`01`, 3.3 ya tiene un dominio llamado D7 — Competencia** (fixture, partidos y resultados), y `10`, 4.7 lo usa como título de sección. Dos cosas distintas con el mismo nombre en el mismo set es exactamente lo que este documento evita en D-83 ("el club no es la organización") y en `04`, 5.5 ("la división no es una tercera categoría"). **`Certamen`** es además la palabra que usa el fútbol argentino para el evento completo, así que no hay que inventar vocabulario. **La etiqueta visible sigue siendo el nombre que le puso el organizador** —"Apertura 2026"—: la palabra "certamen" no aparece en la interfaz.

**[Definido — D-103] Esto no reabre D-19b.** El Certamen agrupa las **divisiones de un mismo evento**, que corren en paralelo; las **ediciones** —Apertura 2026 después de Apertura 2025— corren en serie y **siguen sin modelarse**. Son dos ejes distintos y confundirlos sería el error: si la misma entidad tuviera que agrupar las dos cosas, "Apertura" contendría seis torneos sin forma de saber cuáles juegan entre sí. Cuando las ediciones entren, entran **encima** del Certamen, no adentro.

**Qué cambia en el set (ya aplicado):** `03` suma la entidad `CERTAMEN` y la sección 3.23, y `TORNEO` suma `certamen_id` y `division`; `04` suma la división en 5.5 y el motivo de rechazo en 4.15; en `02`, UC-16 suma el alta de divisiones, UC-22 el agrupamiento, UC-24 la advertencia de doble inscripción y UC-25 el rechazo con sugerencia; `05` lo refleja en el flujo del organizador y en el del capitán; `08` describe la tarjeta agrupada; `10` suma el servicio y el código de error; y `11` lo incorpora a **T2** (tabla y restricciones), **T9** (`agregarDivision`), **T12** (rechazo y aviso) y **T22** (agrupamiento del listado).
