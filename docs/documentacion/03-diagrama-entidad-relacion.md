...
| `fecha_inicio_estimada` / `fecha_fin_estimada` | Insumo de la decisión de inscribirse y del filtro de descubrimiento. Son estimadas: en el fútbol amateur las fechas reales las define el fixture. **[Definido — D-99]** De la distancia entre las dos **se deriva la duración del torneo** —un día, fin de semana o liga extendida (`04`, 5.4)—, y un torneo de **3 días o menos es un relámpago**. **No hay campo ni bandera que lo declare**: un indicador que pudiera contradecir a las fechas sería un estado de más, y el organizador ya carga las dos. De esa derivación dependen el filtro de duración (UC-22) y el acortamiento de los plazos de confirmación (`06`, D-100). |
