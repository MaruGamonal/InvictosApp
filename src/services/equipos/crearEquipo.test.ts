...
describe('crearEquipo', () => {
  ...
  it('con la cuenta sin confirmar, CUENTA_NO_CONFIRMADA', async () => {
    mockearDb({ perfilId: '21111111-1111-1111-1111-111111111111', cuentaConfirmada: false });
    const { crearEquipo } = await import('./crearEquipo');
    await expect(
      crearEquipo({ nombre: 'Los Pibes FC', categoriaGenero: 'male' }, contextoCon('usuario-1')),
